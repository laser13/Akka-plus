 ...
 *  - Inside our route definition, we instance a new `ru.pavlenov.HelloHandler` actor and pass the event to it
 *  - The `ru.pavlenov.HelloHandler` actor is executed in Akka default thread pool. This frees up the Netty thread pool to
 *    undertake more networking activities.
 */
object Main extends Logger {
  //
  // STEP #1 - Define Actors and Start Akka
  // See `ru.pavlenov.HelloHandler`
  //
  val actorSystem = ActorSystem("HelloExampleActorSystem")

  //
  // STEP #2 - Define Routes
  // Dispatch all HTTP GET events to a newly instanced `ru.pavlenov.HelloHandler` actor for processing.
  // `ru.pavlenov.HelloHandler` will `stop()` itself after processing each request.
  //
  val routes = Routes({
//    case GET(request) =>
...
//        request.response.write(HttpResponseStatus.NOT_FOUND)
  ...
  def main(args: Array[String]) {
    ...
    webServer.start()
    println(myWebServerConfig2.hostname, myWebServerConfig2.port)
